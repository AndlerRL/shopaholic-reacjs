...
const orders = props => {
  ...
  let orders = (
    <div className={css.NoOrders}>
      ...
      <p>Make yours so it can appear in here.</p>
    </div>
  );
          ...
          <div>
            <div>
              <p>
                Shipped on: 
                <span style={{
                  color: order.status === 2 ? '#64b5f6' : '#ff1744'
                }}>{' '}
                  { order.status === 2 ? 'Checking Payment' : 'Not delivered yet' }
                </span>
              </p>
              <p>
                Order Status: { order.status === 0 ? 'Not payed' : 'Already payed!' }
              </p>
            </div>
            <div>
              <Btn
